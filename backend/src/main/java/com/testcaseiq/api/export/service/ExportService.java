...
public class ExportService {
    ...
    private static final String XRAY_CSV_REVIEW_WARNING =
            "Review required before Jira/Xray import. This is a draft field mapping generated by TestCaseIQ.";
    private static final String AZURE_DEVOPS_CSV_REVIEW_WARNING =
            "Review required before Azure DevOps import. This is a draft field mapping generated by TestCaseIQ.";
    ...
    public ExportResult exportStory(UUID storyId, ExportFormat format) {
        ...
        String filename = switch (format) {
            case POSTMAN -> "story-%s-approved-api-tests.postman_collection.json".formatted(storyId);
            case XRAY_CSV -> "story-%s-approved-tests-xray.csv".formatted(storyId);
            case AZURE_DEVOPS_CSV -> "story-%s-approved-tests-azure-devops.csv".formatted(storyId);
            default -> "story-%s-approved-test-cases.%s".formatted(storyId, format.extension());
        };
    ...
    public ExportResult exportTestSuite(UUID testSuiteId, ExportFormat format) {
        ...
        String filename = switch (format) {
            case POSTMAN -> "test-suite-%s-approved-api-tests.postman_collection.json".formatted(testSuiteId);
            case XRAY_CSV -> "test-suite-%s-approved-tests-xray.csv".formatted(testSuiteId);
            default -> "test-suite-%s-approved-test-cases.%s".formatted(testSuiteId, format.extension());
        };
        return export(story, format, filename, document);
    }
    ...
    private ExportResult export(Story story, ExportFormat format, String filename, TestCaseExportDocument document) {
        ...
        try {
            String content = switch (format) {
                ...
                case XRAY_CSV -> toXrayCsv(document);
                case AZURE_DEVOPS_CSV -> toAzureDevOpsCsv(document);
    ...
    private String toXrayCsv(TestCaseExportDocument document) {
        List<String> lines = new ArrayList<>();
        lines.add(String.join(",",
                "Test Case ID",
                "Summary",
        ...
        for (ApprovedTestCaseExport testCase : document.testCases()) {
            lines.add(java.util.Arrays.asList(
                    ...
                    csv(join(testCase.linkedRequirementReferences())),
                    csv(join(testCase.sourceEvidence())),
                    csv(flattenStepActions(testCase.steps())),
                    csv(flattenExpectedResults(testCase)),
                    csv(xrayLabels(testCase)),
                    csv(testCase.reviewStatus()),
                    csv(testCase.automationCandidate()),
                    csv(XRAY_CSV_REVIEW_WARNING)
            ).stream().collect(Collectors.joining(",")));
        }
    ...
    private String toAzureDevOpsCsv(TestCaseExportDocument document) {
        List<String> lines = new ArrayList<>();
        lines.add(String.join(",",
                "Test Case ID",
                "Title",
        ...
        for (ApprovedTestCaseExport testCase : document.testCases()) {
            lines.add(java.util.Arrays.asList(
                    ...
                    csv(testCase.reviewStatus()),
                    csv(testCase.automationCandidate()),
                    csv(exportTags(testCase)),
                    csv(AZURE_DEVOPS_CSV_REVIEW_WARNING)
            ).stream().collect(Collectors.joining(",")));
        }
    ...
    private String flattenStepActions(List<ExportedTestStep> steps) {
        return steps.stream()
                .map(step -> "%d. %s".formatted(step.order(), nullToEmpty(step.action())))
                .collect(Collectors.joining("\n"));
    }

    private String flattenExpectedResults(ApprovedTestCaseExport testCase) {
        String expectedResults = testCase.steps().stream()
                .filter(step -> hasText(step.expectedResult()))
                .map(step -> "%d. %s".formatted(step.order(), step.expectedResult()))
                .collect(Collectors.joining("\n"));
        if (hasText(expectedResults)) {
            return expectedResults;
        }
        return hasText(testCase.bddScenario()) ? "1. " + testCase.bddScenario() : "";
    }

    private String xrayLabels(ApprovedTestCaseExport testCase) {
        return exportTags(testCase);
    }

    private String exportTags(ApprovedTestCaseExport testCase) {
        List<String> labels = new ArrayList<>();
        labels.add("draft-mapping");
        labels.add("testcaseiq");
        labels.add("story-" + (hasText(testCase.storyReference()) ? testCase.storyReference() : testCase.storyId()));
        if (hasText(testCase.testSuiteName())) {
            labels.add("suite-" + testCase.testSuiteName());
        }
